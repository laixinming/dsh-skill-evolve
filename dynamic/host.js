// ============================================================================
// Skill 全域自演化架构 — Host 半（引擎）
// 基于 v5.0 规格说明书实现；纯 JS，无 import。由 cordis_define code.host 使用。
// 入口函数体：return { inject: ['fs','timer'], async apply(ctx) { ... } }
// ============================================================================
return {
  inject: ['fs', 'timer'],
  async apply(ctx) {
    ...
    function createEngine(root) {
      ...
      async function wrText(p, c) {
        const t = await fs.resolve(p);
        // 显式声明策略：演化系统需要写入其根目录（部署级策略即 danger-full-access）
      ...
      async function removeDir(p) {
        ...
        if (sub) {
          try {
            ...
            // 注意：cwd 不能是要删除的目录本身（Windows 上进程 CWD 会锁住目录导致部分删除静默失败）
      ...
      // 移除标记语义：.removed 存在且非空 → 视为已删除；空内容 → 视为存活
      ...
      // ---------------------------------------------------------------- 工作区调度
      const mounted = new Map(); // wsPath -> { wsId, mountedAt }
      ...
      async function runPromotionPoolGC() {
        ...
        // 相似片段去重：同 origin_skill_id，内容 Levenshtein 相似度 > 0.9 保留最新
      ...
      async function runSelfTest() {
        ...
        const seedNoise = async (from, to, count) => {
          const payloads = [];
          for (let i = 0; i < count; i++) {
            payloads.push({ skillId: 'noise-skill', scope: 'workspace', success: true, time: from + ((to - from) * (i / count)) });
          }
          await eng.recordCallsBulkPublic(payloads);
        };
        ...
        // ---- TC-20 自动生成权重衰减
        await T('TC-20', '权重衰减：auto_generated 技能频率 ×0.5', async () => {
      ...
      return {
        ...
        // 生命周期
        ensureInit, resetInstance,
        // 测试
        runSelfTest,
        // 别名（测试用）
        setNowPublic: setNow, hashStrPublic: hashStr, recordCallPublic: recordCall, recordCallsBulkPublic: recordCallsBulk, resolveSkillPublic: resolveSkill, checkPromotionPublic: checkPromotion,
        checkDemotionPublic: checkDemotion, executePromotionPublic: executePromotion, executeDemotionPublic: executeDemotion,
    ...
    // 每日聚合 + 凌晨 3 点 GC 的轻量轮询
    let lastDaily = null;
    const timerDisp = ctx.interval(() => {
    ...
    // ------------------------------------------------------------ RPC（Client→Host）
    const dispRpc = [];
    const handle = (method, fn) => { dispRpc.push(harness.handle(method, fn)); };
    handle('evolve.state', async () => engine.overview());
    handle('evolve.detail', async (args) => engine.skillDetail(args && args.name));
    handle('evolve.action', async (args) => engine.executeAction(args && args.notificationId, args && args.actionId));
    handle('evolve.review', async (args) => engine.reviewAction(args && args.reviewId, args && args.action));
    handle('evolve.config.update', async (args) => engine.saveConfig(args && args.patch || {}));
    handle('evolve.notifications', async (args) => engine.listNotifications(args || {}));
    handle('evolve.notifications.markRead', async (args) => engine.markNotifRead(args && args.id));
    handle('evolve.notifications.markAllRead', async () => engine.markAllRead());
    handle('evolve.workspaces', async () => {
      const out = [];
      for (const ws of engine.mountedKeys()) {
        out.push({ wsPath: ws, count: (await engine.listWsSkills(ws)).length });
      }
      return { mounted: out };
    });
    handle('evolve.selftest', async () => engine.runSelfTest());
    handle('evolve.demo', async (args) => runDemo(engine, args && args.scenario || 'promote'));

    // ------------------------------------------------------------ 动态工具
    const tools = [];
    function makeTool(name, description, params, run) {
      ...
      tools.push(harness.registerTool(ctx, tool));
    }
    makeTool('evolve_status', '查看 Skill 全域自演化系统状态（计数、热度、待审核、配置、通知）', undefined,
      async () => engine.overview());
    makeTool('evolve_selftest', '运行 Skill 演化系统完整自验证测试套件（约 23 个典型场景），返回逐项结果', undefined,
      async () => engine.runSelfTest());
    makeTool('evolve_resolve', '按 引导→全局→工作区 顺序解析一个请求命中的 Skill', { type: 'object', properties: { query: { type: 'string', description: '用户诉求/查询文本' }, workspace: { type: 'string', description: '工作区路径（可选）' } }, required: ['query'] },
      async (args) => {
    ...
    makeTool('evolve_run', '执行（模拟）一个 Skill：解析命中、记录计量、返回指令摘要', { type: 'object', properties: { name: { type: 'string', description: 'Skill 名称' }, workspace: { type: 'string', description: '工作区路径（可选）' }, success: { type: 'boolean', description: '执行是否成功，默认 true' } }, required: ['name'] },
      async (args) => {
    ...
    makeTool('evolve_demo', '在主实例上触发一个典型演化场景（promote/demote/generate/split/lock/rollback），用于演示通知卡片与仪表盘联动', { type: 'object', properties: { scenario: { type: 'string', description: 'promote | demote | generate | split | lock | rollback' } }, required: ['scenario'] },
      async (args) => runDemo(engine, args.scenario || 'promote'));

    // ------------------------------------------------------------ 演示场景（主实例）
    async function runDemo(eng, scenario) {
      ...
      if (scenario === 'generate') {
        const r = await eng.detectMiss('帮我生成项目周报', demoWs);
      ...
      if (scenario === 'lock') {
        ...
        await eng.sendNotification({ type: 'info', skillId: 'demo-lock-skill', title: 'Skill "demo-lock-skill" 已锁定', message: 'user_locked=true，升级/降级/自动演化均跳过该 Skill', actions: [{ id: 'dismiss', label: '知道了' }], dedupKey: 'lock:demo-lock-skill' });
    ...
    return () => {
      timerDisp();
      for (const d of dispRpc) d();
      for (const d of tools) d();
    };
  }
};
